...
class YT01AddDevicesVC: RootViewController, UITableViewDelegate, UITableViewDataSource, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didConnect peripheral: CBPeripheral) {
        ...
        peripheral.discoverServices([CBUUID.init(string: "FFF0")])
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        if let services = peripheral.services {
            for service in services {
                ...
                peripheral.discoverCharacteristics([CBUUID.init(string: "FFF2")], for: service)
            }
        } else {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        if let characteristics = service.characteristics {
            for characteristic in characteristics {
                YT01Manager.shared.deviceCharMap[service.peripheral] = characteristic
    ...
    override func TableViewConfig() {
        self.tableView.register(UINib.init(nibName: "YT01AddDevicesTVC", bundle: nil), forCellReuseIdentifier: "YT01AddDevicesTVC")
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "YT01AddDevicesTVC") as! YT01AddDevicesTVC
        cell.peripheral = self.deviceArr[indexPath.row]
        cell.AddBtnClickClosure = {
            if let peripheral = cell.peripheral {
                YT01Manager.shared.addedDeviceArr.append(peripheral)
